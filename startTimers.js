...
export function startTimer() {
  ...
  const oninactivityNew = (inactivityTimeMs) => {
    waiterNew.cancel()
    document.querySelector("#new").innerHTML += `${shortTime(new Date())}-${sec(inactivityTimeMs)}<br/>`
  ...
  const oninactivityOld = (inactivityTimeMs) => {
    waiterOld?.cancel()
    document.querySelector("#old").innerHTML += `${shortTime(new Date())}-${sec(inactivityTimeMs)}<br/>`
